...
split=1
directory_path = "/home/roucoulab/Desktop/SLURM_TMPDIR/checkpointing-split1"
...
try:
    print('found checkpointing')
    entries = os.listdir(directory_path)
    num = [int(i.split('-')[-1]) for i in entries]
    check=max(num)
    peft_config = PeftConfig.from_pretrained(f'/home/roucoulab/Desktop/SLURM_TMPDIR/checkpointing-split1/{entries[0].split('-')[0]}-{check}')
    model1=AutoModelForTokenClassification.from_pretrained(peft_config.base_model_name_or_path)
    model = PeftModel.from_pretrained(model1, f'/home/roucoulab/Desktop/SLURM_TMPDIR/checkpointing-split1/{entries[0].split('-')[0]}-{check}', is_trainable=True)

except:
    print('found no checkpointing')
    check=0
    ...
    model = get_peft_model(model1, peft_config)
    model.to(device)
...
train, val, test=get_Data("/home/roucoulab/Desktop/nutr_tutorial/nutr_data_test.csv", ',', 'sequence', 'labels', tokenizer, chrm_split, 1, check)

#decide step and save strategy
steps_per_epoch = len(train)
print(f'steps_per_epoch  {steps_per_epoch}')
save_eval_freq = steps_per_epoch // 10

train_args = TrainingArguments(
                    output_dir=f"/home/roucoulab/Desktop/SLURM_TMPDIR/checkpointing-split{split}",
